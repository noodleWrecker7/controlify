...
function buttonPressShuffle() {
    makeCall("PUT", "https://api.spotify.com/v1/me/player/shuffle", "state=" + !player.shuffle).then(response => {
...
function buttonPressBackward() {
    makeCall("POST", "https://api.spotify.com/v1/me/player/previous").then(response => {
        console.log(response)
        getPlayerInfo();
    });
}
...
function buttonPressPlay() {
    if (player.is_playing) {
        makeCall("PUT", "https://api.spotify.com/v1/me/player/pause").then(response => {
            console.log(response)
        });
    } else {
        makeCall("PUT", "https://api.spotify.com/v1/me/player/play").then(response => {
            console.log(response)
        });
...
function buttonPressForward() {
    makeCall("POST", "https://api.spotify.com/v1/me/player/next").then(response => {
        console.log(response)
        getPlayerInfo();
    });
}
...
async function getPlayerInfo() {
    makeCall("GET", "https://api.spotify.com/v1/me/player").then(value => {
        ...
        document.getElementById("track-length").innerText = msToTime(player.track.length);

        player.track.progress = data.progress_ms;
        document.getElementById("current-progress").innerText = msToTime(player.track.progress);
        ...
        document.getElementById("album-image").setAttribute("src", player.track.album_image)
    })
}
...
function displayControlBar() {
    ...
    document.getElementById("progress").addEventListener("mousemove", updateProgressBar)
    document.getElementById("progress").addEventListener("click", updateProgressBar);

    for (let item in controls) {
        document.getElementById(controls[item].id).style.opacity = "1";
        document.getElementById(controls[item].id).addEventListener("click", controls[item].action);
    }
...
function removeControlBar() {
    ...
    document.getElementById("progress").removeEventListener("mousemove", updateProgressBar)
    document.getElementById("progress").removeEventListener("click", updateProgressBar);

    for (let item in controls) {
        document.getElementById(controls[item].id).style.opacity = "0";
        document.getElementById(controls[item].id).removeEventListener("click", controls[item].action);
    }
}
...
function displayControlsPanel() {
    ...
    document.addEventListener("mousemove", function () {
        displayControlBar();
        clearInterval(timerToRemovePanel);
...
function updateProgressBar() {
    ...
    var percent = val * 100;
    //console.log(this)
    document.getElementById("progress").style.cssText = 'background-image: -webkit-gradient(linear, left top, right top, ' +
        'color-stop(' + percent + '%, #00d06f), ' +
        'color-stop(' + percent + '%, #737373)' +
        ');'
        + 'background-image: -moz-linear-gradient(left center, #00d06f 0%, #00d06f ' + percent + '%, #737373 ' + percent + '%, #737373 100%)';
    if (!player.controlsViewed) {
        document.getElementById("progress-bar-container").style.opacity = 0;
    }
}
